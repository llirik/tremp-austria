...
export const load: PageServerLoad = async (event) => {
	...
	const [{ profile, contact }, ridesResult, requests] = await Promise.all([
...
export const actions: Actions = {
	saveProfile: async (event) => {
		...
		const { error: profileError } = await supabase.from('profiles').upsert({ id: user.id, display_name: parsed.display_name });
		if (profileError) return fail(400, { error: actionError(profileError, 'לא הצלחנו לשמור את השם. נסו שוב.'), values });
		const { error: contactError } = await supabase.from('private_contacts').upsert({ user_id: user.id, ...parsed.contact });
