...
import { withRequestContacts } from '$lib/server/request-contacts';
...
export const load: PageServerLoad = async (event) => {
	...
	const withContacts = await withRequestContacts(supabase, user.id, requests);
	return {
		profile,
		contact,
		legalAccepted,
		rides: (ridesResult.data || []).map((ride) => ({
			...ride,
			display_name: profile?.display_name || 'חבר/ת הקהילה'
		})) as Ride[],
...
export const actions: Actions = {
	saveProfile: async (event) => {
		...
		if (profileError)
			return fail(400, {
				error: actionError(profileError, 'לא הצלחנו לשמור את השם. נסו שוב.'),
				values
			});
		...
		if (contactError)
			return fail(400, { error: 'השם נשמר, אך פרטי הקשר לא נשמרו. בדקו אותם ונסו שוב.', values });
	...
	respond: async (event) => {
		...
		if (!validId(requestId) || !['accepted', 'rejected'].includes(status))
			return fail(400, { error: 'הבקשה אינה תקינה.' });
		if (status === 'accepted') await requireLegalAcceptance(supabase, '/account');
		const { error: responseError } = await supabase.rpc('respond_contact_request', {
		...
		return {
			success:
				status === 'accepted'
					? 'הבקשה אושרה. כעת אפשר לראות את פרטי הקשר של שני הצדדים.'
					: 'הבקשה נדחתה. פרטי הקשר נשארו פרטיים.'
		};
	},
	revoke: async (event) => {
		const { supabase } = requireUser(event);
		...
		const { error: revokeError } = await supabase.rpc('revoke_contact_request', {
			p_request_id: requestId
		});
		if (revokeError) return fail(400, { error: 'לא הצלחנו לבטל את הגישה. נסו שוב.' });
		return {
			success:
				'האישור לבקשה הזו בוטל. בקשות אחרות שאושרו ביניכם עדיין מאפשרות גישה. פרטים שכבר הועתקו אינם ניתנים למחיקה.'
		};
	},
