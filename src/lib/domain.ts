...
export function safeNext(value: string | null | undefined, fallback = '/account'): string {
	...
	try {
		...
		if (
			parsed.origin !== 'https://tremp.invalid' ||
...
export function validatePublicText(value: string): string | null {
	if (
		...
		return 'הטקסט מכיל תו שאינו נתמך. יש להסיר תווים מיוחדים ולנסות שוב.';
	if (
...
export function parseRide(form: FormData, now = new Date()): { data?: RideInput; error?: string } {
	...
	if (!Object.hasOwn(rideTypes, type) || !Object.hasOwn(directions, direction))
		return { error: 'יש לבחור סוג נסיעה וכיוון.' };
	...
	try {
		departure = localDeparture(date, time);
	} catch {
		return { error: 'השעה אינה חד־משמעית בגלל מעבר שעון. יש לבחור שעה אחרת.' };
	}
	const delta = new Date(departure).getTime() - now.getTime();
	if (delta < 0 || delta > 366 * 86400000)
		return { error: 'יש לבחור מועד עתידי במהלך השנה הקרובה.' };
	const flex = get('flexibility_minutes');
	if (!['0', '30', '60', '120', 'flexible', ''].includes(flex))
		return { error: 'יש לבחור גמישות תקינה.' };
	...
	if (
		...
		passengers < 1 ||
		passengers > 8 ||
		(seats !== null && (!Number.isInteger(seats) || seats < 0 || seats > 8))
	)
		return {
			error: 'מספר הנוסעים או המקומות חייב להיות בין 1 ל־8 (אפשר לסמן רכב מלא עם 0 מקומות).'
		};
	...
	if (origin.length < 2 || destination.length < 2 || origin.length > 80 || destination.length > 80)
		return { error: 'יש להזין אזור איסוף ויעד, בין 2 ל־80 תווים.' };
	if (note.length > 280) return { error: 'ההערה יכולה להכיל עד 280 תווים.' };
	for (const text of [origin, destination, note]) {
		const error = validatePublicText(text);
		if (error) return { error };
	}
	const flight = get('flight_number').toUpperCase();
	if (flight && !/^[A-Z0-9]{2,3}\s?\d{1,4}[A-Z]?$/.test(flight))
		return { error: 'מספר הטיסה אינו תקין. לדוגמה: W6 1234.' };
	return {
		data: {
			ride_type: type as RideType,
			direction: direction as Direction,
			departure_at: departure,
			flexibility_minutes: flex === '' || flex === 'flexible' ? null : Number(flex),
...
export function isPotentialMatch(a: Ride, b: Ride): boolean {
	if (
		...
		return false;
	const compatible =
		(a.ride_type === 'taxi' && b.ride_type === 'taxi') ||
		(a.ride_type === 'driver' &&
			b.ride_type === 'passenger' &&
			(a.available_seats ?? 0) >= b.passenger_count) ||
		(b.ride_type === 'driver' &&
			a.ride_type === 'passenger' &&
			(b.available_seats ?? 0) >= a.passenger_count);
	if (!compatible) return false;
	if (a.flexibility_minutes === null || b.flexibility_minutes === null)
		return localDate(a.departure_at) === localDate(b.departure_at);
	return (
		Math.abs(Date.parse(a.departure_at) - Date.parse(b.departure_at)) <=
		(a.flexibility_minutes + b.flexibility_minutes) * 60000
	);
}
export function potentialMatches(ride: Ride, rides: Ride[]): Ride[] {
	return rides.filter((candidate) => isPotentialMatch(ride, candidate));
}
export function shareText(ride: Ride, url: string): string {
	const route = directions[ride.direction];
	const count =
		ride.ride_type === 'driver'
			? `יש ${ride.available_seats} מקומות פנויים`
			: ride.ride_type === 'taxi'
				? `${ride.passenger_count} נוסעים מחפשים שותפים למונית`
				: `${ride.passenger_count} נוסעים מחפשים טרמפ`;
